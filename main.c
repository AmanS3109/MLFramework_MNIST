...
int main(void) {
...

    {
        ...
        for (u32 i = 0; i < 60000; i++) {
            u32 num = (u32)(train_labels_file->data[i] + 0.5f);
            train_labels->data[i * 10 + num] = 1.0f;
        }

        for (u32 i = 0; i < 10000; i++) {
            u32 num = (u32)(test_labels_file->data[i] + 0.5f);
...
matrix* mat_load(mem_arena* arena, u32 rows, u32 cols, const char* filename) {
    ...
    size_t read = fread(mat->data, 1, size, f);
    if (read != size) {
        // optional: zero remaining bytes or log
    }
    ...
    for (u32 epoch = 0; epoch < training_desc->epochs; epoch++) {
        ...
        for (u32 batch = 0; batch < num_batches; batch++) {
            ...
            for (u32 i = 0; i < model->cost_prog.size; i++) {
                ...
                mat_scale(
                    cur->grad,
                    training_desc->learning_rate / (f32)training_desc->batch_size
                );
        ...
        printf(
            ...
            num_correct, num_tests, (f32)num_correct / (f32)num_tests * 100.0f,
            avg_cost
        );
    }
